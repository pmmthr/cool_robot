...
class Mapping:
    ...
    def __init__(self, width, height, resolution):
        ...
        # Inverse sensor model probabilities (Table 9.2)
        self.p_occ = 0.9    
        self.p_free = 0.3   
        

        # Obstacle thickness α (Table 9.2)
    ...
    def update_with_scan(self, pose, theta, z_max, walls, n_beams=10):
        """
        ...
        for ang in angles:
            ...
            # “for all cells mᵢ in perceptual field of this beam”: (occupancy grid mapping algorithm p226 table 9.1)
            ...
            for k in range(max_steps+1):
                ...
                if not (0 <= row < self.rows and 0 <= col < self.cols):
                    continue
                # get P(mᵢ|z,x) and convert to log-odds
    ...
    def draw(self, screen):
        """
        Convert log-odds, to probability with Eq.(9.6) p225. Use shades of color=(1-p)*255.
        """
        for r in range(self.rows):
            for c in range(self.cols):
                l = self.log_odds[r, c]
                p = 1.0 -(1.0/ (1.0 + math.exp(l)) ) # Eq. 9.6, p225
